...
public class CityInfoService implements CityInfoInterface {
    ...
    @Autowired CityInfoRepository cityInfoRepository;
    ...
    public CityData fetchCityByName(String cityName) {
        //Fetching city details by city name
        System.out.println("Fetched city details by name from DB");
        if(cityInfoRepository.existsByCityName(cityName)){
            return cityInfoRepository.findByCityName(cityName);
        }
        else
            throw new CityInfoException("Get operation aborted since, City Name doesn't exist in database..");
    }
